    ...
    body: JSON.stringify({
      ...
      html: `
        <div style="font-family:Arial,sans-serif;max-width:560px;margin:auto;padding:32px;color:#222">
          <h2>Reset your HotelEase password</h2>
...
app.post('/api/auth/forgot-password', authRateLimit, async (req, res) => {
  try {
    ...
    if (user.auth_provider && user.auth_provider !== 'password') {
      return res.json({ message: 'This account uses social/phone sign-in. Please use that sign-in method instead.' });
    }
    ...
    try {
      await sendPasswordResetEmail({ to: user.email, name: user.name, url: resetPasswordUrl(rawToken) });
    } catch (mailError) {
...
app.post('/api/auth/reset-password', authRateLimit, (req, res) => {
  try {
    ...
    if (password.length < 8 || !/[A-Z]/.test(password) || !/[a-z]/.test(password) || !/\\d/.test(password)) {
      return res.status(400).json({ message: 'Password must be at least 8 characters and include an uppercase letter, lowercase letter, and number.' });
    }
